...
bool check(char* word)
{
    char index_of_word = word[0];


    int i = 0;
    node *ptr = root;
    while (index_of_word != '\0' && ptr != NULL)
    {
        ptr = ptr->children[HashChar(word[i])];
        if (ptr == NULL)
        {
            return false;
        }
...
int HashChar(char letter)
{
    int deductionAmount = 65;

    int index = 0;

    if (letter >= 97)
    {
        index = letter - (32 + deductionAmount);
    } else
    {
        index = letter - deductionAmount;
    }
